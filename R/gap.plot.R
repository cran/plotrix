gap.plot<-function(x,y,gap.bounds,gap.axis="y",xaxlab,xtics,yaxlab,ytics,
 col=par("col"),...) {
 if(missing(y) && !missing(x)) {
  y<-x
  x<-1:length(y)
 }
 if(missing(gap.bounds)) stop("gap.bounds must be specified")
 gapsize<-gap.bounds[2]-gap.bounds[1]
 ...
 if(gap.axis == "y") {
  littleones<-which(y<=gap.bounds[1])
  bigones<-which(y>=gap.bounds[2])
  if(any(y > gap.bounds[1] & y < gap.bounds[2]))
   warning("gap includes some values of y")
  xlim<-range(x)
  ylim<-c(min(y),max(y)-gapsize)
 }
 else {
  littleones<-which(x<=gap.bounds[1])
  bigones<-which(x>=gap.bounds[2])
  if(any(x > gap.bounds[1] & x < gap.bounds[2]))
