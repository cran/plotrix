plotCI<-function(x,y=NULL,uiw,liw=uiw,ui=NULL,li=NULL,err="y",sfrac=0.01,
 gap=0,slty=par("lty"),add=FALSE,scol=NULL,pt.bg=par("bg"),...) {
 ...
 if(!add)
  do.call("plot",c(list(x,y,type="n"),clean.args(arglist,plot)))
  ...
  if(err == "y") {
   ...
   arrow.args<-c(list(lty=slty,angle=90,length=smidge,code=1,
    col=scol),clean.args(arglist,arrows,exclude.other=c("col","lty")))
   do.call("arrows",c(list(x,li,x,pmax(y-gap,li)),arrow.args))
   do.call("arrows",c(list(x,ui,x,pmin(y+gap,ui)),arrow.args))
  }
  else if(err=="x") {
   ...
   do.call("arrows",c(list(li,y,pmax(x-gap,li),y),arrow.args))
   do.call("arrows",c(list(ui,y,pmin(x+gap,ui),y),arrow.args))
  }
  if(plotpoints)
   do.call("points",c(list(x,y,bg=pt.bg),
    clean.args(arglist,points,
    exclude.other=c("xlab","ylab","xlim","ylim","axes"))))
