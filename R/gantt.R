...
gantt.chart<-function (x = NULL, format = "%Y/%m/%d", xlim = NULL, taskcolors = NULL, 
    ...
    vgrid.format = "%Y/%m/%d", half.height = 0.25, hgrid = FALSE, 
    main = "", xlab = "", cylindrical = FALSE, label.cex = 1, 
    border.col = NA, priority.label = "Priorities", priority.extremes = c("High", 
        "Low"), time.axis = 3) 
{
    oldpar <- par("mai", "omi", "xpd", "xaxs", "yaxs")
    if (is.null(x)) 
        x <- get.gantt.info(format = format)
    if (any(x$starts > x$ends)) 
        stop("Can't have a start date after an end date")
    tasks <- unique(x$labels)
    ntasks <- length(tasks)
    if (is.null(x$priorities)) 
        x$priorities <- rep(1, ntasks)
    if (is.null(dev.list())) 
        plot.new()
    ...
    if (is.null(taskcolors)) 
        taskcolors <- barcolors <- rainbow(npriorities)
    else barcolors <- taskcolors
    if (length(barcolors) < ntasks) 
        barcolors <- barcolors[x$priorities]
    ...
    plot(range(x$starts), c(1, ntasks), xlim = xlim, ylim = c(0.5, 
        ntasks + 0.5), main = "", xlab = "", ylab = "", axes = FALSE, 
        type = "n")
    box()
    if (nchar(main)) 
        mtext(main, 3, 2, at = getFigCtr()[1])
    if (nchar(xlab)) 
        mtext(xlab, 1, 1)
    if (is.na(vgrid.format)) {
        ...
        else axis(time.axis, at = tickpos, labels = vgridlab, 
            padj = 0)
    }
    ...
    for (i in 1:ntasks) {
        if (cylindrical) 
            ...
            half.height, x$ends[x$labels == tasks[i]], topdown[i] + 
            half.height, col = barcolors[x$labels == tasks[i]], 
            border = border.col)
    }
    if (hgrid) 
    ...
    if (priority.legend) {
        ...
        color.legend(plim[1] + 2 * plot.width/5, plim[3] - (6.5 - 
            time.axis) * line.height, plim[1] + 3 * plot.width/5, 
            plim[3] - (5.5 - time.axis) * line.height, legend = c("High", 
                rep("", length(taskcolors) - 2), "Low"), rect.col = taskcolors)
        par(xpd = NA)
        text(plim[1] + plot.width/2, plim[3] - (7 - time.axis) * 
            line.height, priority.label, adj = c(0.5, 1))
        par(xpd = FALSE)
    }
    par(oldpar)
    invisible(x)
