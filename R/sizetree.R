sizetree<-function (x,left=0,top,right=1,lastcenter=NA,showval=TRUE, 
 showcount=TRUE,firstcall=TRUE,col=NA,colorindex=1,...) {
 ...
 if(firstcall) {
  ...
  par(mar=c(1,2,2,1))
  top<-sum(!is.na(x[,1]))
  if(top<dimx[1]) cat(dimx[1]-top,"NA values dropped from first stack.\n")
 ...
 if(is.list(col)) {
  ...
  # if it's a list, set the index to the next element for the next call
  ...
  if(length(xfreq[bar])) {
   if(!is.na(xfreq[bar])) {
    if(xfreq[bar] > 0) {
     ...
     cex<-
      ifelse((1.5*labelheight)>xfreq[bar],0.75*xfreq[bar]/labelheight,1)
     if(showval) {
      textcol<-ifelse(colSums(col2rgb(unlist(barcol[bar]))*c(1.4,1.4,0.5))<350,
       "white","black")
      bartext<-
       ifelse(showcount,paste(labels[bar]," (",xfreq[bar],")",sep=""),labels[bar])
      text((left+right)/2,top-xfreq[bar]/2,bartext,cex=cex,col=textcol)
     }
     if(!is.na(lastcenter)) 
