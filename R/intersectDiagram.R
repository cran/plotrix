intersectDiagram<-function(x,pct=FALSE,show.nulls=FALSE,xnames=NULL, 
 ...
 if(length(col) < length(x))
  col<-c(rep(col,length.out=length(x)-1),NA)
 ...
 for(comb in 1:lenx) {
  ...
  nonzero<-which(x[[comb]]>0)
  lenxcomb<-length(nonzero)
  gap<-horizmax/lenxcomb
  startx<-gap/2
  for(intersect in nonzero) {
   cellqnt<-ifelse(pct,paste(round(100*x[[comb]][intersect]/xtotal,1),
    "%",sep=""),x[[comb]][intersect])
 ...
 if(show.nulls) {
  ...
  xpos<-leftx+nonset/2
  if(pct) nonset<-paste(round(100*nonset/xtotal,1),"%",sep="")
